...
# ---- Precheck: verify Excel can actually open a file ----
    $knownGood = Get-ChildItem -Path $TestDir -Recurse -Filter "01-basic-single.xlsb" | Select-Object -First 1
if ($knownGood) {
    try {
...
$files = Get-ChildItem -Path $TestDir -Filter "*.xls?" -Recurse | Sort-Object FullName
$processed = 0
$passed = 0
$failed = 0
...
foreach ($file in $files) {
    ...
    try {
        ...
        $passed++
        $msg = "PASS|$rel|$sheetCount sheets|${ms}ms"
        Write-Output $msg
